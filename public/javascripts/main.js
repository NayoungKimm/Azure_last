...
function DropFile(dropAreaId, fileListId) {
    ...
    fileDOM.innerHTML = `
      <div class="details">
        <div class="detailFile">
          ...
          <button class="submitButton file-label-1">제출</button>
        </div>
      </div>
    `;
  ...
  function showFilePreview(file) {
    console.log("File to preview1:", file);
    if (file) {
      console.log("File to preview2:", file);
      var preview = URL.createObjectURL(file);
      document.getElementById("preview").src = preview;

      // 이 부분은 #modal 요소가 있는지 확인해야 합니다.
      let modalElement = document.getElementById("modal");
      if (modalElement) {
        modalElement.style.display = "flex"; // 모달을 보여줍니다.
      } else {
        console.warn("modal element is not found");
      }
    } else {
...
window.onload = function () {
  ...
  document
    ...
    .addEventListener("click", function () {
      ...
      fetch("http://localhost:2200/upload", {
        method: "POST",
        body: formData,
      })
        .then((response) => {
